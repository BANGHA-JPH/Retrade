...
export default function CallsScreen() {
  ...
  const filteredCalls = calls.filter(call => {
    ...
    if (selectedFilter === 'missed') return call.type === 'missed';
    return call.type !== 'missed';
  });

  const getCallIcon = (call) => {
    if (call.type === 'missed') {
      return call.callType === 'video' ? 'videocam-off' : 'call-missed';
    }
    return call.callType === 'video' ? 'videocam' : 'call';
  };

  const getCallColor = (call) => {
    if (call.type === 'missed') return '#FF3B30';
    if (call.type === 'incoming') return '#007bff';
    return '#007bff'; // outgoing
  };
      ...
      <View style={styles.callContent}>
        ...
        <View style={styles.callFooter}>
          <Text style={styles.callType}>
            {getCallDirection(item)} • {item.callType === 'video' ? 'Video Call' : 'Voice Call'}
          </Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.filterTabs}>
        ...
        <TouchableOpacity 
          style={[
            styles.filterTab,
            selectedFilter === 'missed' && styles.activeFilterTab
          ]}
          onPress={() => setSelectedFilter('missed')}
        >
          <Text style={[
            styles.filterTabText,
            selectedFilter === 'missed' && styles.activeFilterTabText
          ]}>
            Missed
          </Text>
        </TouchableOpacity>
